import { Alert, AlertIcon, Box, Button, ButtonGroup, Input, InputGroup, InputLeftElement, ring } from "@chakra-ui/react";
...
const CartOrderForm = ({ onBack, onSend }: Props) => {
    ...
    const { isValid: isEnteredNameValid, isInvalid: isNameInputInvalid, errorText: nameErrorMessage } = validateInput(enteredName, false, false, false, wasNameInputTouched);
    const { isValid: isEnteredEmailValid, isInvalid: isEmailInputInvalid, errorText: emailErrorMessage } = validateInput(enteredEmail, false, true, false, wasMailInputTouched);
    const { isValid: isEnteredPhoneValid, isInvalid: isPhoneInputInvalid, errorText: phoneErrorMessage } = validateInput(enteredPhone, true, false, false, wasPhoneInputTouched);
    const { isValid: isEnteredAddressValid, isInvalid: isAddressInputInvalid, errorText: addressErrorMessage } = validateInput(enteredAddress, false, false, true, wasAddressInputTouched);
    ...
    return <form className={styles.form} onSubmit={onSubmitFormHandler}>
    {inputs.map((input, index) => {
        return <InputGroup key={index} className={styles.inputContainer}>
    ...
    <Button colorScheme='green' type='submit'>Make order</Button>
   </ButtonGroup>
